...
struct Year2024Day3: ParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "3",
    abstract: "Day 3 Challenge",
    shouldDisplay: true,
    aliases: ["Day3"]
  )
  ...
  mutating func run() throws {
    ...
    // Part 1
    do {
      let ops = try MulOpListParser().parse(input)
      let result = ops.reduce(0) { $0 + $1.evaluate() }
    ...
    // Part 2
    do {
      let ops = try OpListParser().parse(input).filter {
        switch $0 {
      ...
      for op in ops {
        switch op {
        case .dontOp: enabled = false
        case .doOp: enabled = true
        case .mulOp(let left, let right):
          if enabled {
            result += left * right
          }
        case .noOp: break
  ...
  struct MulOpParser: Parser {
    var body: some Parser<Substring, MulOp> {
      OneOf {
        Parse(MulOp.init) {
          Skip {
            PrefixThrough("mul(")
